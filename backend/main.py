...
logger = get_json_logger("backend")
...
UPLOAD_DIR = config.get("UPLOAD_DIR", "../downloads")
...
async def process_url(request: VideoUrlRequest):
    try:
        ...
        queue.append({
            ...
            "golden_video_url": request.golden_video_url,
        ...
        return {"status": "success", "video_id": video_id, "message": "URL queued for processing."}
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Could not queue URL: {str(e)}")

# New direct upload endpoint for local bypassing of S3
@app.post("/upload")
async def upload_file(file: UploadFile = File(...), user_id: str = Form("user123"), golden_video_url: str = Form("")):
    try:
        video_id = str(uuid.uuid4())
        ...
        queue.append({
            ...
            "golden_video_url": golden_video_url,
            "user_id": user_id,
